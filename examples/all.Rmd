...
for (df in dfs) {
  ...
  res <- deconv(nmix,nsignatures,markers=sel_xmarkers$id, method = 'deconRNASeq')
  ...
  res$tissue.ordered <- factor(df$tissue, levels = unique.names, ordered = T)
  
  save(file = paste0(res, '-deconRNASeq.RData'), res)
